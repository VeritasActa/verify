...
export function formatRunManifestResult(result, opts = {}) {
  ...
  if (result.provenance) lines.push(`  Provenance: ${result.provenance.verified ? green('verified against the pinned Sigstore trust root') : red('supplied, does not verify or does not name these bytes')}${result.provenance.identity?.workflow ? `; ${result.provenance.identity.workflow.replace(/^https:\/\/github\.com\//, '')}${result.provenance.identity.commit ? ` at ${result.provenance.identity.commit.slice(0, 12)}` : ''}` : ''}${result.provenance.log ? `; ${result.provenance.log.base_url.replace(/^https:\/\//, '')} index ${result.provenance.log.index} (${result.provenance.log.integrated_time})` : ''}`);
  if (result.model_attestation) lines.push(`  Model route: ${result.model_attestation.verified ? green(`attested: ${result.model_attestation.calls} calls signed inside ${result.model_attestation.provider}'s TEE for ${result.model_attestation.model}, ${result.model_attestation.reports} report(s) verified to Intel's root`) : yellow(`attested by ${result.model_attestation.provider} (${result.model_attestation.calls} signed calls); add --model-calls and --model-attestation to verify`)}`);
