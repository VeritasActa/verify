...
import { verifyRepositoryEvidence, verifyRepositoryCollaborationEvidence } from './legate-core.mjs';
export async function verifyRepositoryArtifact(input, opts = {}) {
  const collaboration = input?.type === 'scopeblind.repository.collaboration-evidence.v1';
  const result = await (collaboration ? verifyRepositoryCollaborationEvidence : verifyRepositoryEvidence)(input, opts.publicKey);
  const state = (collaboration ? input?.repository : input)?.state?.payload;
  return {
    ...result,
    format: 'repository-evidence',
    artifact_type: collaboration ? 'scopeblind.repository.collaboration-evidence.v1' : 'scopeblind.repository.evidence.v1',
