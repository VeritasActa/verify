...
test('actual CLI verifies additive review packets and refuses stripped exact packet decisions',async()=>{
 const f=await fixture(),s=f.evidence.state.payload,time=n=>new Date(1800000000000+n*1000).toISOString(),brief=await sign({type:'scopeblind.repository.review-brief.v1',task_id:s.task.payload.id,task_digest:s.task.digest,owner_key:f.owner.key,brief:'A functioning client website.',success_criteria:[{id:'contact-works',text:'The contact form submits.'}],issued_at:time(0),expires_at:s.task.payload.expires_at},f.owner);
 const packet=await sign({type:'scopeblind.repository.review-packet.v1',task_id:s.task.payload.id,task_digest:s.task.digest,brief_digest:brief.digest,proposal_digest:s.proposal.digest,base_sha:s.proposal.payload.base_sha,head_sha:s.proposal.payload.head_sha,merge_sha:s.proposal.payload.merge_sha,preview:{status:'not_requested',reason:'No external preview was requested.'},observed_at:time(2),expires_at:time(902)},f.receiver);
 const decisions=await Promise.all(s.approvals.map(a=>sign({type:'scopeblind.repository.review-decision.v1',task_id:s.task.payload.id,task_digest:s.task.digest,brief_digest:brief.digest,packet_digest:packet.digest,proposal_digest:s.proposal.digest,approval_digest:a.digest,principal_key:a.payload.principal_key,role:a.payload.role,issued_at:a.payload.issued_at,expires_at:a.payload.expires_at},a.payload.role==='owner'?f.owner:f.reviewer)));
 const review=await sign({type:'scopeblind.repository.review-state.v1',task_id:s.task.payload.id,task_digest:s.task.digest,brief,packet,decisions,feedback:[],recommendations:[],history:[],agent_uses:[],observed_at:time(8)},f.service),evidence={type:'scopeblind.repository.review-evidence.v1',repository:f.evidence,review};
 assert.equal(detectFormat(evidence).mode,'repository-evidence');const dir=await mkdtemp(join(tmpdir(),'scopeblind-review-packet-cli-'));
 try{const file=join(dir,'review.json');async function run(value,key=f.service.key){await writeFile(file,JSON.stringify(value));const result=spawnSync(process.execPath,['cli.js',file,'--key',key,'--json'],{cwd:new URL('../..',import.meta.url),encoding:'utf8'});return{status:result.status,report:JSON.parse(result.stdout),diagnostic:result.stderr+result.stdout};}
  const good=await run(evidence);assert.equal(good.status,0,good.diagnostic);assert.equal(good.report.artifact_type,evidence.type);assert.equal(good.report.packetVerified,true);assert.equal(good.report.decisionsVerified,true);assert.equal(good.report.accepted,true);assert.equal(good.report.previewAvailable,false);assert.match(good.report.not_established.join(' '),/URL is mutable/);
  for(const bad of [{...evidence,review:await sign({...review.payload,decisions:[]},f.service)},{...evidence,review:await sign({...review.payload,brief:await sign({...brief.payload,brief:'Changed requirements'},f.owner)},f.service)}]){const checked=await run(bad);assert.notEqual(checked.status,0,checked.diagnostic);assert.equal(checked.report.valid,false);assert.equal(checked.report.accepted,false);assert.equal(checked.report.decisionsVerified,false);}
  assert.notEqual((await run(evidence,f.owner.key)).status,0);
 }finally{await rm(dir,{recursive:true,force:true});}
});
