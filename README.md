...
| Legate run manifest | A harness's signed account of a governed run (`scopeblind.run_manifest.v1`), such as a benchmark submission made with every tool call through the gateway. Add `--standard standard.json --receipts receipts.jsonl` to check the pins, the policy, the tool list, the attempts, the time limit, and the chain head against the standard and the receipts; `--calls calls.jsonl` opens each receipt to its call, `--regrade regrade.json` reconciles a second grading (repeatable: the run's own first, then any grading made elsewhere, accepted by its key or by the provenance identity the standard names), `--provenance provenance/` verifies the run's Sigstore provenance bundles offline against the pinned trust root (workflow, commit, run, log entry) and ties them to the exact bytes of the files given, and `--model-calls model-calls.jsonl --model-attestation model-attestation.json` verifies that every model call was signed inside the inference provider's TEE and that each signing key is bound in an Intel TDX quote that chains to the pinned Intel root | T1 |
